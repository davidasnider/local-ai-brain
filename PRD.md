...
## 3. Core Models
* **Text/Reasoning/Vision (LLM):** Qwen 3.6 (e.g., 27B parameter) quantized for MLX (4-bit or 8-bit to respect RAM limits). Must use a patched `vllm-mlx` implementation (via `llm_server.py`) to chunk prefill operations and prevent macOS Metal watchdog timeouts.

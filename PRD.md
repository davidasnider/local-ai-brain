...
* **Agent Skills:** Provide agent skills (in `.agents/skills`) like `bump-version`, `restart-dev`, `run-dev`, `tail-logs`, and `vllm-update` to streamline local development workflows.
  * *Note:* The development server skills (`run-dev`, `restart-dev`) run on port `8888` to avoid conflicting with the production port `8000`. These development skills only start the API Gateway proxy and do not start the downstream backend microservices (LLM on port `8001`, STT on port `8002`, TTS on port `8003`). For inference to work, these backend services must be running independently via `local-brain serve`.

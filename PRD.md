...
## 3. Core Models
* **Text/Reasoning/Vision (LLM):** Qwen 3.6 (e.g., 35B parameter) quantized for MLX (4-bit to respect RAM limits). Must use a patched `vllm-mlx` implementation (via `src/local_ai_brain/models/llm_server.py`) to chunk prefill operations (`prefill_step_size=128`), restrict max sequences (`max_num_seqs=1`), and serialize requests at the API gateway level (via semaphore) to completely prevent macOS Metal watchdog timeouts.

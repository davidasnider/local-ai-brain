...
4. **Logging (Crucial):**
   ...
   * Set `LOG_PROMPTS=true` in the environment to log a preview of the last message in the request payload, replacing `\n` and `\r` with spaces. Truncated output is strictly capped to never exceed 100 characters in total, including an appended ellipsis. When `LOG_PROMPTS=false`, the application emits `DEBUG`-level log lines for redacted chat requests to preserve production observability.

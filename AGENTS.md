...
10. **LLM Execution & GPU Timeout Prevention:**
    * Always run `vllm-mlx` via the custom wrapper located at `src/local_ai_brain/models/llm_server.py`. This wrapper applies essential monkeypatches that default `prefill_step_size` to 128 and `max_num_seqs` to 1 when those values are not explicitly set, reducing macOS Metal watchdog timeout risk during large model prefill operations on Apple Silicon.

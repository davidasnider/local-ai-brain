...
## Monitoring
Metrics are exposed at `/metrics` in Prometheus format, covering HTTP requests, active LLM requests, token consumption and generation, processing latency, and precise system and process RAM usage. Backend metrics (vLLM, STT, TTS) are fetched concurrently to minimize latency.

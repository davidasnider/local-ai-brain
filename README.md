...
- **Observability:** Granular logging with `loguru` directly to file and a robust Prometheus `/metrics` endpoint which fetches backend metrics concurrently. Set `LOG_PROMPTS=true` in the environment to log a 100-character preview of the last user message (`\n` replaced with spaces). Enabling this may write potentially sensitive user-provided content to logs; use with caution.

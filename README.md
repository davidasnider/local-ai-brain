...
- **Observability:** Granular logging with `loguru` directly to file and a robust Prometheus `/metrics` endpoint. Set `LOG_PROMPTS=true` in the environment to log the full chat prompt. Enabling this logs full prompts containing potentially sensitive content to the file; use with caution.

...
It uses a microservices architecture: a FastAPI API Gateway proxy sits in front of dedicated `llama-cpp-python` (LLM) (with the configuration identifier `VLLM_URL` retained for backwards compatibility), Whisper (STT), and Kokoro (TTS) backend services, exposing a unified OpenAI-compatible interface.

...
- **Observability:** Granular logging with `loguru` directly to file and a robust Prometheus `/metrics` endpoint. Set `LOG_PROMPTS=true` in the environment to log a 100-character preview of the last user message content (with newlines replaced). Enabling this logs prompt previews containing potentially sensitive content to the file; use with caution.

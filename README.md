...
- **Observability:** Granular logging with `loguru` directly to file and a robust Prometheus `/metrics` endpoint. Set `LOG_PROMPTS=true` in the environment to log a 100-character preview of the last user message (`\n` and `\r` replaced with spaces, with an ellipsis appended if truncated). Enabling this may write potentially sensitive user-provided content to logs; use with caution.

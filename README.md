...
- **TTS (Text-to-Speech):** Kokoro TTS via ONNX with custom dynamic voice routing (configurable via `KOKORO_HF_REPO`, `KOKORO_ONNX_FILE`, and `KOKORO_VOICES_FILE`). Input length is restricted by the `TTS_MAX_CHARACTERS` setting (defaults to 4096).

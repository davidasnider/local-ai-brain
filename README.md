...
- **Observability:** Granular logging with `loguru` directly to file and a robust Prometheus `/metrics` endpoint. Set `LOG_PROMPTS=true` in the environment to log a preview (first 100 characters) of the last message/prompt, not the full prompt. Enabling this logs a preview containing potentially sensitive content to the file; use with caution.

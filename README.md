...
- **Observability:** Granular logging with `loguru` directly to file and a robust Prometheus `/metrics` endpoint. Set `LOG_PROMPTS=true` in the environment to log a truncated preview (approx. 100 characters) of the *last* message/prompt only. Enabling this logs sensitive content to the file; use with caution.

...
- **Observability:** Granular logging with `loguru` directly to file and a robust Prometheus `/metrics` endpoint. Set `LOG_PROMPTS=true` in the environment to log a 100-character preview of the last message in the request payload (`\n` replaced with spaces). Enabling this may write potentially sensitive user-provided content to logs; use with caution.
- **Security:** Authenticated via a static `LOCAL_API_KEY` Bearer token. HTTP request methods and paths are sanitized to protect against CRLF log injection attacks, using a fast conditional check to avoid unnecessary string allocations.
...
## Monitoring
Metrics are exposed at `/metrics` in Prometheus format, covering HTTP requests, active LLM requests, token consumption and generation, processing latency, and precise system and process RAM usage. These metrics are fetched concurrently from the underlying microservices.

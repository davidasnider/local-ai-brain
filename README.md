...
- **LLM (Text/Reasoning/Vision):** Qwen 3.6 35B quantized for MLX (4-bit). The API is backed by the canonical 4-bit model variant; clients should use canonical 4-bit model IDs for inference requests rather than legacy `*-8bit` aliases. To reduce macOS Metal watchdog timeout risk during large prefill operations, the system defaults `prefill_step_size` to 128 and `max_num_seqs` to 1 via application settings. Concurrent requests are serialized at the API Gateway level.

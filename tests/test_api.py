...
def test_get_model_vllm(mock_proxy_request, client):
    ...
    response = client.get(
        "/v1/models/some-vllm-model", headers={"Authorization": "Bearer test-secret-key"}
    )
    assert response.status_code == 200
...
def test_proxy_stt(mock_send, client):
    ...
    response = client.post(
        "/v1/audio/transcriptions", headers={"Authorization": "Bearer test-secret-key"}
    )
    assert response.status_code == 200
...
def test_proxy_chat(mock_send, client):
    ...
    response = client.post(
        "/v1/chat/completions", headers={"Authorization": "Bearer test-secret-key"}
    )
    assert response.status_code == 200
